 ...
 * IMPORTANT: This is the version you will be graded on
 */
char smooth_descr[] = "smooth: 保存重复利用的结果";
void smooth(int dim, pixel *src, pixel *dst) 
{
	...
	for(j=0;j<dim; j++)
	{
		...
		for(i=1;i<dim-1; ++)
		{
			...
			dst[RIDX(i, j, dim)].blue = (unsigned short)((rowsum[i-1][j].blue+rowsun[i][j].blue+rowsun[i+1][j].blue )/snum);
			dst[RIDX(i, j，dim)].green = (unsigned short)((rowsum[i-1][j].green+rowsum[i][j].green+r owsum[i+1][j].green)/snum);
		}
		snum =rowsum[dim-1][j].num+ rowsum[dim-1][j].num;
